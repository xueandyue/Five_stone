...
EOF;
	...
	if($row['status']==0){
		...
		$res=$PdoMySQL->find($table,"username='{$username}' AND password='{$password}'",'id');
		...
		$query=$PdoMySQL->update($data,$table,"id='{$id}' ");
		 $_SESSION['username']=$username;
		
		
		echo '登陆成功,3秒钟后跳转到首页';
		echo '<meta http-equiv="refresh" content="3;url=user.php?id='.$id.'"/>';
	}
